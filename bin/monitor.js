...
async function verifyRuntime(rootDir = path.resolve(__dirname, '..'), env = process.env) {
  ...
  if (config.captchaProvider === 'dashscope') {
    checks.push({ name: '验证码识别（千问）DASHSCOPE_API_KEY', ok: Boolean(env.DASHSCOPE_API_KEY) });
  } else {
    const tessFile = path.join(config.tessdataDir, 'eng.traineddata.gz');
    const ready = fs.existsSync(tessFile);
    checks.push({
      name: ready ? '本地 OCR 语言包 tessdata' : '本地 OCR 语言包缺失（运行 npm run tessdata，否则将回退在线加载）',
      ok: ready,
      optional: true,
    });
  }
