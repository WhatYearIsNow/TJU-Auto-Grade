...
function parseCommand(text) {
  ...
  if (/(?:\bstatus\b|状态|zt|运行)/i.test(value)) return 'status';
  if (/(总加权|加权|jq|平均|\bhistory\b)/i.test(value)) return 'weighted';
  if (/(?:\blog\b|日志|rz|变化)/i.test(value)) return 'log';
  if (/(成绩单|本学期|学期|成绩|cj)/i.test(value)) return 'grades';
  if (/(?:\bhelp\b|帮助|hz|命令|指令)/i.test(value)) return 'help';
  return null;
}
      ...
      socket.once('secureConnect', async () => {
        try {
          ...
          for (const uid of uids) {
            ...
            const command = parseCommand(text.replace(token, ''));
            if (command) {
