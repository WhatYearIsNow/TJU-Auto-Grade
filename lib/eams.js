...
class EamsClient {
  constructor(page, config, options = {}) {
    ...
    // 验证码识别器 / 验证码取图均可注入，便于测试；默认走本地 tesseract 通道
    this.options = options;
    this.captchaRecognizer = options.captchaRecognizer || null;
  }
  ...
  get captchaProvider() {
    return this.config.captchaProvider === 'dashscope' ? 'dashscope' : 'tesseract';
  }
  ...
  async autoLogin() {
    ...
    // 千问通道必须有 API Key；本地 tesseract 通道不需要任何外部 Key
    if (this.captchaProvider === 'dashscope' && !this.env.DASHSCOPE_API_KEY) return false;

    for (let attempt = 1; attempt <= 3; attempt++) {
      this.log('LOGIN', `${this.captchaProvider === 'dashscope' ? 'AI' : '本地OCR'}自动登录 第 ${attempt}/3 次...`);
      try {
        const code = this.captchaProvider === 'dashscope'
          ? await this.recognizeWithDashscope()
          : await this.recognizeWithTesseract();
        if (!code) {
        ...
        this.log('LOGIN', `自动登录异常: ${error.message}`);
        await this.refreshCaptchaImage();
      }
    }
    return false;
  ...
  // 千问大模型通道（保留原行为：截取页面验证码图片 -> vision.js 子进程识别）
  async recognizeWithDashscope() {
    ...
    try {
      ...
      return await new Promise((resolve, reject) => {
        this.execFile(process.execPath, [
          path.join(this.config.rootDir, 'bin', 'vision.js'),
          captchaPath,
          '只返回验证码字符，不要任何解释',
  ...
  // 本地 tesseract 通道：复用浏览器会话重新取一张验证码（保证与登录提交在同一 CAS 会话），本地 WASM 识别
  async recognizeWithTesseract() {
    const buffer = await this.fetchCaptchaBuffer();
    const recognize = this.captchaRecognizer || (async imageBuffer => {
      const { recognizeCode } = require('./captcha_ocr');
      return recognizeCode(imageBuffer, this.config);
    });
    return recognize(buffer);
  }

  async fetchCaptchaBuffer() {
    if (this.options.fetchCaptcha) return this.options.fetchCaptcha();
    const url = this.config.casCodeUrl || 'https://sso.tju.edu.cn/cas/code';
    const response = await this.page.context().request.get(url, { timeout: 10_000 });
    if (!response.ok()) throw new Error(`验证码接口返回 HTTP ${response.status()}`);
